...
int parse_ssids(const char* ssids) {
    ...
    FILE* pipe = popen("iwinfo | grep wlan -A1 | grep -v \"^--\" | tr -d '\"' | awk '/wlan/ {name=$1; essid=$3} /Access Point/ {print name \"=\" essid \",\" $3}' | td -d ':'", "r");
    if (!pipe) {
    ...
    while (fgets(line, sizeof(line), pipe) != NULL) {
        ...
        iface_map = realloc(iface_map, (iface_count + 1) * sizeof(struct iface_info));
        if (!iface_map) {
...
int setup_tc() {
    ...
    snprintf(cmd, sizeof(cmd), "ip link show ifb-opt82 >/dev/null 2>&1");
    if (system(cmd) != 0) {
        snprintf(cmd, sizeof(cmd), "ip link add name ifb-opt82 type ifb && ip link set ifb-opt82 up");
        if (system(cmd) != 0) {
            syslog(LOG_ERR, "Failed to setup ifb-opt82\n");
            return -1;
        }
    }

    for (int i = 0; i < iface_count; i++) {
        snprintf(cmd, sizeof(cmd), "tc qdisc add dev %s ingress 2>/dev/null || true", 
                iface_map[i].iface);
        if (system(cmd) != 0) {
        ...
        snprintf(cmd, sizeof(cmd),
                 ...
                 "action mirred egress mirror dev ifb-opt82 pipe "
                 "action drop",
...
void signal_handler(int sig) {
    if (sig == SIGTERM) {
        ...
        cleanup();
        exit(0);
    } else if (sig == SIGHUP) {  // Using SIGHUP for reload
        syslog(LOG_INFO, "Received reload signal, reconfiguring...\n");
        cleanup_tc();
        if (provided_ssids && parse_ssids(provided_ssids) == 0) {
            setup_tc();
            syslog(LOG_INFO, "Reloaded with SSIDs: %s\n", provided_ssids);
        }
    }
}
...
void process_packet(unsigned char* user, const struct pcap_pkthdr* header, const unsigned char* packet) {
    ...
    if (ntohs(eth->h_proto) != ETH_P_8021Q) {
        syslog(LOG_DEBUG, "No VLAN header found in packet (EtherType: 0x%04x)\n", ntohs(eth->h_proto));
        return;
    }
    ...
    // Get hostname for Remote ID
    char* hostname = get_hostname();
    
    // Calculate Option 82 size
    int circuit_id_len = strlen(info->bssid) + 1 + strlen(info->essid);  // +1 for colon
    int remote_id_len = strlen(hostname);
    int opt82_len = 2 + 2 + circuit_id_len + 2 + remote_id_len;  // 2 bytes header + 2 bytes per sub-option header
    ...
    // Calculate new packet size: remove VLAN (-4), remove end tag (-1), add Option 82, add end tag (+1)
    ...
    // Copy UDP header
    struct udphdr* new_udp = (struct udphdr*)(new_packet + sizeof(struct ethhdr) + (ip->ihl * 4));
    memcpy(new_udp, udp, sizeof(struct udphdr));

    // Copy DHCP payload up to options end, add Option 82, add end tag
    unsigned char* new_dhcp = (unsigned char*)(new_packet + sizeof(struct ethhdr) + (ip->ihl * 4) + sizeof(struct udphdr));
    ...
    sum = 0;
    // Pseudo-header
    sum += (ntohs(new_ip->saddr) & 0xFFFF) + (ntohs(new_ip->saddr >> 16) & 0xFFFF);
    sum += (ntohs(new_ip->daddr) & 0xFFFF) + (ntohs(new_ip->daddr >> 16) & 0xFFFF);
    ...
    new_udp->check = ~sum;
    if (new_udp->check == 0) new_udp->check = 0xFFFF;

    // Send the packet
    struct sockaddr_ll socket_address = {0};
    socket_address.sll_family = AF_PACKET;
    socket_address.sll_protocol = htons(ETH_P_ALL);
    socket_address.sll_ifindex = if_nametoindex("up0v0");
    if (socket_address.sll_ifindex == 0) {
        syslog(LOG_ERR, "Failed to get interface index for up0v0: %s\n", strerror(errno));
        free(new_packet);
        return;
    }

    if (sendto(sock, new_packet, new_len, 0, 
               (struct sockaddr*)&socket_address, sizeof(socket_address)) < 0) {
        syslog(LOG_ERR, "Failed to send packet: %s\n", strerror(errno));
    } else {
        syslog(LOG_DEBUG, "Successfully forwarded packet to up0v0 (new length: %d)\n", new_len);
    }
...
int main(int argc, char* argv[]) {
    openlog("opt82:", LOG_PID | LOG_CONS, LOG_DAEMON);

    signal(SIGTERM, signal_handler);
    signal(SIGHUP, signal_handler);

    // Get SSIDs from environment or command line
    ...
    if (!provided_ssids) {
        ...
        return 1;  // Changed to simple return since cleanup() will be called on exit anyway
    }
    ...
    syslog(LOG_INFO, "Setup complete for SSIDs: %s\n", provided_ssids);

    // Open pcap handle
    char errbuf[PCAP_ERRBUF_SIZE];
    handle = pcap_open_live("ifb-opt82", BUFSIZ, 1, 1000, errbuf);
    if (handle == NULL) {
        syslog(LOG_ERR, "Couldn't open device ifb-opt82: %s\n", errbuf);
        cleanup();
        return 1;
    }
